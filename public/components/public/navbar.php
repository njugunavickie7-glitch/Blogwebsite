...
.topbar{
    ...
    background:var(--brand-primary,#0D9488); color:#fff;
    font-family:var(--font-body,sans-serif); font-size:.8rem;
}
...
.topbar-link{ display:inline-flex; align-items:center; gap:8px; color:#fff; opacity:.95; transition:opacity .2s ease; }
.topbar-link:hover{ opacity:.75; }
.topbar-link i{ font-size:.78rem; }
.topbar-loc{ display:inline-flex; align-items:center; gap:8px; opacity:.95; }
.topbar-loc i{ font-size:.78rem; }
...
.site-header.is-scrolled{ box-shadow:0 4px 24px rgba(10,52,47,.08); }

.header-inner{ display:flex; align-items:center; gap:var(--space-6,24px); height:100%; }

/* Brand */
.header-brand{ display:flex; align-items:center; gap:12px; flex-shrink:0; text-decoration:none; }
.brand-mark{ display:flex; align-items:center; justify-content:center; width:44px; height:44px; flex-shrink:0; }
.brand-mark img{ width:44px; height:44px; object-fit:contain; }
.brand-text{ display:flex; flex-direction:column; line-height:1.05; }
.brand-name{ font-family:var(--font-display,sans-serif); font-size:1.18rem; font-weight:800; letter-spacing:.02em; color:var(--color-text-heading,#0A1413); }
.brand-tag{ font-family:var(--font-body,sans-serif); font-size:.68rem; font-weight:600; letter-spacing:.18em; text-transform:uppercase; color:var(--color-text-muted,#7B8987); }

/* Desktop nav with dropdown */
.header-nav{ margin-left:auto; }
.nav-list{ display:flex; align-items:center; gap:2px; list-style:none; margin:0; padding:0; }
.nav-link{ display:inline-flex; align-items:center; gap:7px; font-family:var(--font-body,sans-serif); font-size:.875rem; font-weight:500; color:var(--color-text-body,#44524F); padding:9px 14px; border-radius:var(--radius-md,12px); text-decoration:none; white-space:nowrap; position:relative; transition:color .2s ease, background .2s ease; }
.nav-link:hover{ color:var(--brand-primary,#0D9488); background:rgba(13,148,136,.07); }
.nav-link.is-active{ color:var(--brand-primary,#0D9488); font-weight:600; }
.nav-link.is-active::after{ content:''; position:absolute; bottom:3px; left:14px; right:14px; height:2px; background:var(--brand-primary,#0D9488); border-radius:2px; }
.nav-ic{ font-size:.85em; }
.nav-pill{ font-size:.6rem; font-weight:700; letter-spacing:.04em; text-transform:uppercase; color:#fff; background:var(--brand-primary,#0D9488); border-radius:var(--radius-full,9999px); padding:2px 7px; }

/* Store Dropdown Styles */
.nav-item-dropdown{ position:relative; }
.dropdown-arrow{ font-size:.7rem; margin-left:4px; transition:transform .2s ease; }
.nav-item-dropdown:hover .dropdown-arrow{ transform:rotate(180deg); }
.dropdown-menu{ position:absolute; top:calc(100% + 8px); left:0; min-width:220px; background:var(--color-surface,#fff); border:1px solid var(--color-border,#E2EAE8); border-radius:var(--radius-md,12px); box-shadow:var(--shadow-lg); padding:8px 0; opacity:0; visibility:hidden; transform:translateY(-8px); transition:all .2s ease; z-index:100; list-style:none; margin:0; }
.nav-item-dropdown:hover .dropdown-menu{ opacity:1; visibility:visible; transform:translateY(0); }
.dropdown-link{ display:flex; align-items:center; gap:10px; padding:8px 16px; font-size:.85rem; color:var(--color-text-body,#44524F); text-decoration:none; transition:background .2s ease, color .2s ease; }
.dropdown-link:hover{ background:rgba(13,148,136,.08); color:var(--brand-primary,#0D9488); }
.dropdown-link i{ width:18px; font-size:.85rem; color:var(--color-text-muted,#7B8987); }
.dropdown-link:hover i{ color:var(--brand-primary,#0D9488); }
.dropdown-divider{ border:none; border-top:1px solid var(--color-border,#E2EAE8); margin:5px 0; }
.cart-badge{ display:inline-block; background:var(--brand-primary,#0D9488); color:#fff; font-size:.65rem; font-weight:600; padding:2px 6px; border-radius:20px; margin-left:8px; min-width:20px; text-align:center; }

/* Cart Icon Mobile */
.cart-icon-mobile{ display:none; align-items:center; justify-content:center; width:40px; height:40px; border:1px solid var(--color-border,#E2EAE8); background:transparent; border-radius:var(--radius-md,12px); color:var(--color-text-muted,#7B8987); cursor:pointer; transition:all .2s ease; position:relative; text-decoration:none; }
.cart-icon-mobile:hover{ color:var(--brand-primary,#0D9488); border-color:var(--brand-primary,#0D9488); }
.cart-count-mobile{ position:absolute; top:-5px; right:-8px; background:#e74c3c; color:#fff; font-size:.65rem; font-weight:700; padding:2px 5px; border-radius:20px; min-width:18px; text-align:center; }

/* Actions */
.header-actions{ display:flex; align-items:center; gap:var(--space-3,12px); flex-shrink:0; }
.theme-toggle{ display:inline-flex; align-items:center; justify-content:center; width:40px; height:40px; border:1px solid var(--color-border,#E2EAE8); background:transparent; border-radius:var(--radius-md,12px); color:var(--color-text-muted,#7B8987); cursor:pointer; transition:all .2s ease; }
.theme-toggle:hover{ color:var(--brand-primary,#0D9488); border-color:var(--brand-primary,#0D9488); }
html[data-theme="dark"] .theme-toggle i::before{ content:"\f185"; } /* sun */
.nav-cta{ box-shadow:0 4px 14px rgba(13,148,136,.25); }

/* User dropdown */
.user-menu{ position:relative; }
.user-trigger{ display:flex; align-items:center; gap:8px; background:var(--color-surface-alt,#F1F6F5); border:1px solid var(--color-border,#E2EAE8); border-radius:var(--radius-full,9999px); padding:5px 12px 5px 5px; cursor:pointer; font-family:var(--font-body,sans-serif); font-size:.875rem; color:var(--color-text-body,#44524F); transition:border-color .2s ease, box-shadow .2s ease; }
.user-trigger:hover{ border-color:var(--brand-primary,#0D9488); box-shadow:var(--shadow-sm); }
.user-avatar{ width:28px; height:28px; border-radius:50%; background:var(--brand-primary,#0D9488); color:#fff; display:flex; align-items:center; justify-content:center; font-size:.75rem; font-weight:700; flex-shrink:0; }
.user-name{ font-weight:500; max-width:90px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.user-caret{ font-size:.6rem; color:var(--color-text-muted,#7B8987); transition:transform .2s ease; }
.user-trigger[aria-expanded="true"] .user-caret{ transform:rotate(180deg); }
.user-dropdown{ position:absolute; top:calc(100% + 8px); right:0; min-width:220px; background:var(--color-surface,#fff); border:1px solid var(--color-border,#E2EAE8); border-radius:var(--radius-md,12px); box-shadow:var(--shadow-xl); padding:6px; opacity:0; pointer-events:none; transform:translateY(-6px) scale(.97); transform-origin:top right; transition:opacity .2s ease, transform .2s ease; z-index:200; }
.user-dropdown.is-open{ opacity:1; pointer-events:all; transform:translateY(0) scale(1); }
.dropdown-item{ display:flex; align-items:center; gap:10px; padding:9px 12px; border-radius:var(--radius-sm,6px); font-size:.875rem; color:var(--color-text-body,#44524F); text-decoration:none; transition:background .2s ease, color .2s ease; }
.dropdown-item:hover{ background:rgba(13,148,136,.08); color:var(--brand-primary,#0D9488); }
.dropdown-item--danger{ color:#d9534f; }
.dropdown-item--danger:hover{ background:#fff5f5; color:#c0392b; }
.dropdown-item i{ width:15px; text-align:center; font-size:.85em; color:var(--color-text-muted,#7B8987); }
.dropdown-item--danger i{ color:#d9534f; }
.dropdown-divider{ border:none; border-top:1px solid var(--color-border,#E2EAE8); margin:5px 0; }

/* Mobile Drawer Submenu */
.drawer-item-parent{ flex-direction:column; }
.drawer-toggle{ cursor:pointer; background:none; border:none; width:100%; text-align:left; }
.drawer-toggle .drawer-chevron{ transition:transform .3s ease; margin-left:auto; }
.drawer-toggle[aria-expanded="true"] .drawer-chevron{ transform:rotate(180deg); }
.drawer-submenu{ list-style:none; margin:0; padding:0; max-height:0; overflow:hidden; transition:max-height .3s ease; background:rgba(13,148,136,.03); }
.drawer-submenu.open{ max-height:400px; }
.drawer-sub-link{ display:flex; align-items:center; gap:10px; padding:12px 24px 12px 48px; font-size:.9rem; color:var(--color-text-body,#44524F); text-decoration:none; border-bottom:1px solid var(--color-border,#E2EAE8); transition:background .2s ease, color .2s ease; }
.drawer-sub-link:hover{ background:rgba(13,148,136,.08); color:var(--brand-primary,#0D9488); }
.drawer-sub-link i{ width:18px; font-size:.85rem; }
.drawer-sub-divider{ margin:5px 0; border-color:var(--color-border,#E2EAE8); }

/* Hamburger */
.hamburger{ display:none; align-items:center; justify-content:center; width:44px; height:44px; background:none; border:1px solid var(--color-border,#E2EAE8); border-radius:var(--radius-md,12px); cursor:pointer; padding:0; flex-direction:column; gap:5px; transition:border-color .2s ease, background .2s ease; flex-shrink:0; }
.hamburger:hover{ border-color:var(--brand-primary,#0D9488); background:rgba(13,148,136,.05); }
.ham-bar{ display:block; width:18px; height:2px; background:var(--color-text-heading,#0A1413); border-radius:2px; transition:transform .3s ease, opacity .3s ease; pointer-events:none; }
.hamburger.is-open .ham-bar--top{ transform:translateY(7px) rotate(45deg); }
.hamburger.is-open .ham-bar--mid{ opacity:0; }
.hamburger.is-open .ham-bar--bot{ transform:translateY(-7px) rotate(-45deg); }

/* Drawer */
.drawer-backdrop{ position:fixed; inset:0; z-index:1099; background:rgba(6,52,47,.55); backdrop-filter:blur(3px); -webkit-backdrop-filter:blur(3px); opacity:0; pointer-events:none; transition:opacity .35s ease; }
.drawer-backdrop.is-open{ opacity:1; pointer-events:all; }
.mobile-drawer{ position:fixed; top:0; right:0; bottom:0; width:min(340px,88vw); z-index:1100; background:var(--color-surface,#fff); display:flex; flex-direction:column; transform:translateX(100%); transition:transform .38s cubic-bezier(.32,0,.15,1); box-shadow:-8px 0 40px rgba(6,52,47,.18); overflow-y:auto; overscroll-behavior:contain; }
.mobile-drawer.is-open{ transform:translateX(0); }
.drawer-head{ display:flex; align-items:center; justify-content:space-between; padding:18px 22px; border-bottom:1px solid var(--color-border,#E2EAE8); flex-shrink:0; }
.drawer-head .brand-name{ font-size:1rem; }
.drawer-close{ display:flex; align-items:center; justify-content:center; width:40px; height:40px; background:var(--color-surface-alt,#F1F6F5); border:1px solid var(--color-border,#E2EAE8); border-radius:var(--radius-md,12px); cursor:pointer; font-size:1rem; color:var(--color-text-body,#44524F); transition:all .2s ease; flex-shrink:0; }
.drawer-close:hover{ background:var(--brand-primary,#0D9488); border-color:var(--brand-primary,#0D9488); color:#fff; }
.drawer-nav{ flex:1; padding:10px 0; overflow-y:auto; }
.drawer-list{ list-style:none; margin:0; padding:0; }
.drawer-item{ opacity:0; transform:translateX(20px); transition:opacity .3s ease calc(var(--i) * 45ms + 80ms), transform .3s ease calc(var(--i) * 45ms + 80ms); }
.mobile-drawer.is-open .drawer-item{ opacity:1; transform:translateX(0); }
.drawer-link{ display:flex; align-items:center; justify-content:space-between; padding:15px 24px; font-family:var(--font-display,sans-serif); font-size:1.1rem; font-weight:700; color:var(--color-text-heading,#0A1413); text-decoration:none; border-bottom:1px solid var(--color-border,#E2EAE8); transition:color .2s ease, background .2s ease; gap:16px; }
.drawer-link:hover, .drawer-link:focus-visible{ color:var(--brand-primary,#0D9488); background:rgba(13,148,136,.05); }
.drawer-arrow{ font-size:.75rem; color:var(--color-text-muted,#7B8987); transition:transform .2s ease, color .2s ease; flex-shrink:0; }
.drawer-link:hover .drawer-arrow{ transform:translateX(4px); color:var(--brand-primary,#0D9488); }
.drawer-foot{ border-top:1px solid var(--color-border,#E2EAE8); padding:22px; background:var(--color-surface-alt,#F1F6F5); flex-shrink:0; }
.drawer-user{ display:flex; align-items:center; gap:12px; margin-bottom:16px; padding:12px; background:var(--color-surface,#fff); border:1px solid var(--color-border,#E2EAE8); border-radius:var(--radius-md,12px); }
.drawer-user-avatar{ width:40px; height:40px; border-radius:50%; background:var(--brand-primary,#0D9488); color:#fff; display:flex; align-items:center; justify-content:center; font-size:1rem; font-weight:700; flex-shrink:0; }
.drawer-user-info{ display:flex; flex-direction:column; }
.drawer-user-name{ font-weight:600; color:var(--color-text-heading,#0A1413); }
.drawer-user-role{ font-size:.75rem; color:var(--color-text-muted,#7B8987); }
.drawer-foot-links{ display:flex; flex-direction:column; gap:2px; }
.drawer-foot-link{ display:flex; align-items:center; gap:10px; padding:10px 12px; border-radius:var(--radius-sm,6px); color:var(--color-text-body,#44524F); text-decoration:none; font-size:.875rem; transition:background .2s ease, color .2s ease; }
.drawer-foot-link i{ width:16px; text-align:center; }
.drawer-foot-link:hover{ background:rgba(13,148,136,.08); color:var(--brand-primary,#0D9488); }
.drawer-foot-link--danger{ color:#d9534f; }
.drawer-foot-link--danger:hover{ background:#fff5f5; color:#c0392b; }
.drawer-foot-tagline{ font-size:.9rem; color:var(--color-text-muted,#7B8987); margin-bottom:14px; }
.drawer-cta{ width:100%; justify-content:center; margin-bottom:14px; }
.drawer-call-link{ display:flex; align-items:center; justify-content:center; gap:8px; font-size:.9rem; font-weight:600; color:var(--brand-primary,#0D9488); text-decoration:none; }

/* Responsive */
@media (max-width:1180px){ .nav-link{ padding:9px 11px; font-size:.84rem; } }
@media (max-width:1024px){ .header-inner{ gap:var(--space-4,16px); } }
@media (max-width:900px){
    ...
    .cart-icon-mobile{ display:inline-flex; }
    .hamburger{ display:inline-flex; }
}
@media (min-width:901px){ .mobile-drawer,.drawer-backdrop{ display:none; } .cart-icon-mobile{ display:none !important; } .hamburger{ display:none !important; } }
@media (max-width:380px){ .brand-name{ font-size:1rem; } .brand-tag{ font-size:.6rem; } }

.hamburger:focus-visible, .drawer-close:focus-visible, .drawer-link:focus-visible, .nav-link:focus-visible, .theme-toggle:focus-visible{ outline:2px solid var(--brand-primary,#0D9488); outline-offset:2px; }
</style>
